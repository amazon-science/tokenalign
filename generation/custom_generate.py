...
from .token_align import (
    apply_allowed_token_mask,
    convert_tokens_to_string_utf8,
    get_pretokenized_position_from_ids,
)
...
def generate(
    ...
    single_context_batch_sampling: Optional[bool] = False,
    token_align: Optional[bool] = False,
    ...
    **kwargs,
    ...
    # BenA: TokenAlign
    # in case TokenAlign failed, we fallback to regular generation
    prompt_suffix_list = None
    use_token_align = token_align
    if token_align:
        try:
            batch_size = input_ids.shape[0]
            pretokenized_positions = [
                get_pretokenized_position_from_ids(input_ids[i], tokenizer, is_llama=tokenizer.is_llama)
                for i in range(batch_size)
            ]
            min_pretokenized_position = min(pretokenized_positions)
            # if vocab is None: vocab = tokenizer.get_vocab()
            prompt_suffix_ids = input_ids[:, min_pretokenized_position:]
            prompt_suffix_tokens_list = [
                tokenizer.convert_ids_to_tokens(prompt_suffix_ids[i])
                for i in range(batch_size)
            ]
            prompt_suffix_list = [
                convert_tokens_to_string_utf8(tokenizer, prompt_suffix_tokens, is_llama=tokenizer.is_llama)
                for prompt_suffix_tokens in prompt_suffix_tokens_list
            ]
            input_ids = input_ids[:, :min_pretokenized_position]
            cur_len = input_ids.shape[-1]

            if not (cur_len >= 0):
                raise ValueError("edge case: input is too short")
            model_kwargs["attention_mask"] = model_kwargs["attention_mask"][
                                             :, :min_pretokenized_position
                                             ]
        except ValueError as e:
            use_token_align = False
            logger.warning(
                f"Can't proceed with token align due to input processing failure, fallback to regular mode.\n{e}"
            )
    ...
    if is_greedy_gen_mode:
        ...
        # expand prompt_suffix_list to batch_size
        if prompt_suffix_list is not None and len(prompt_suffix_list) != input_ids.size(0):
            prompt_suffix_list = [str(prompt_suffix_list[0]) for _ in range(input_ids.size(0))]
        ...
        return sample(
            ...
            synced_gpus=synced_gpus,
            streamer=streamer,
            single_context_batch_sampling=single_context_batch_sampling,
            token_align=use_token_align,
            tokenizer=tokenizer,
            prompt_suffix_list=prompt_suffix_list if use_token_align else None,
            verbose=verbose,
            **model_kwargs,
        )
...
def sample(
    ...
    single_context_batch_sampling: Optional[bool] = False,
    token_align: Optional[bool] = False,
    tokenizer: Optional[bool] = None,
    prompt_suffix_list: Optional = None,
    verbose: Optional[bool] = False,
    **model_kwargs,
    ...
    while True:
        ...
        # BenA: TokenAlign -- masking after logits processing and before warping
        for batch_idx in range(next_token_logits.size(0)):
            if token_align and prompt_suffix_list[batch_idx] != "":
                apply_allowed_token_mask(
                    prompt_suffix_list[batch_idx],
                    tokenizer.vocab_trie,
                    tokenizer.vocab,
                    next_token_logits[batch_idx],
                    single_space_mask=tokenizer.single_space_mask,
                )
                # updating the next_token_logits in place
                apply_allowed_token_mask(
                    prompt_suffix=prompt_suffix_list[batch_idx],
                    vocab_trie=tokenizer.vocab_trie,
                    vocab=tokenizer.vocab,
                    next_token_scores=next_token_scores[0],
                    single_space_mask=tokenizer.single_space_mask,
                )
        # BenA: end of TokenAlign block 1
        ...
        # BenA: TokenAlign -- update 'prompt_suffix'
        if token_align:
            for batch_idx in range(len(next_tokens)):
                prompt_suffix = prompt_suffix_list[batch_idx]
                _next_tokens = next_tokens[batch_idx : batch_idx + 1]

                if verbose and prompt_suffix != "":
                    print(f"batch idx {batch_idx} | prompt suffix =[{prompt_suffix}] | next tokens {tokenizer.convert_ids_to_tokens(_next_tokens)}")
                if prompt_suffix != "":
                    gen_str = convert_tokens_to_string_utf8(tokenizer,
                        tokenizer.convert_ids_to_tokens(_next_tokens.cpu()),
                        is_llama=tokenizer.is_llama
                    )
                    if tokenizer.is_llama:
                        # this is a llama tokenizer!
                        if "<0x0A>" in prompt_suffix:
                            prompt_suffix = prompt_suffix.replace("<0x0A>", "\n")
                        if "<0x0A>" in gen_str:
                            gen_str = gen_str.replace("<0x0A>", "\n")
                    if prompt_suffix.startswith(gen_str):
                        prompt_suffix = prompt_suffix[len(gen_str) :]
                    elif gen_str.startswith(prompt_suffix):
                        prompt_suffix = ""
                    else:
                        raise ValueError(
                            f"Unexpected error. [{prompt_suffix}] and [{gen_str}] should have non empty common prefix"
                        )
                    prompt_suffix_list[batch_idx] = prompt_suffix
        ...
        # BenA: efficient context computation
        # compute context in a single-sample setting and broadcast after the first token
        # in case of constrained generation, wait to broadcast until all character matching is complete
        if (
            single_context_batch_sampling
            and (
                (token_align and prompt_suffix == "")
                or (not token_align and i == 0)
            )
            and input_ids.size(0) == 1
            and model_kwargs.get("num_return_sequences", 1) > 1
        ):
            # print("Expansion at gen str = ", gen_str)
            # print("--------------------------------")
            # at this point prompt suffix has been processed and becomes ""
            # so this current logic works only if there's one rewinding token
            #
            # BenA: needs to handle the case where we have multiple rewinding tokens
            # after initial processing, expand input ids and past key values
            print("After generating: expanding inputs ids with explicit broadcast")
            print("input ids before", input_ids.shape)
            input_ids, model_kwargs = self._expand_inputs_for_generation(
                input_ids,
                expand_size=model_kwargs.get("num_return_sequences"),
                is_encoder_decoder=self.config.is_encoder_decoder,
                **model_kwargs,
            )
            print("input ids after", input_ids.shape)
            expanded_past = None

            def expand_item_first_dim(item, num_refs):
                return item.expand(num_refs, *item.size()[1:])

            # print('type model_kwargs["past"][0][0]', type(model_kwargs["past"][0][0]))
            if not isinstance(model_kwargs["past"][0][0], dict):
                # iterate over all layers
                for ii, item in enumerate(model_kwargs["past"]):
                    expanded_item = (
                        expand_item_first_dim(
                            item[0], model_kwargs.get("num_return_sequences")
                        ),
                        expand_item_first_dim(
                            item[1], model_kwargs.get("num_return_sequences")
                        ),
                    )
                    if expanded_past == None:
                        expanded_past = (expanded_item,)
                    else:
                        expanded_past += (expanded_item,)
                model_kwargs["past"] = expanded_past
        # BenA: end of efficient context computation
